...
// DefiLlama protocol endpoint for Ondo Finance
const ONDO_PROTOCOL_URL = "https://api.llama.fi/protocol/ondo-finance";

type OndoProtocolData = {
  id?: string;
  name?: string;
  chainTvls?: {
...
export async function GET() {
  try {
    const res = await fetch(ONDO_PROTOCOL_URL, {
      next: { revalidate: 300 }, // Cache for 5 minutes
    });

    if (!res.ok) {
      return NextResponse.json(
        { error: "ondo_fetch_failed", status: res.status },
        { status: 502 }
      );
    }

    const data = (await res.json()) as OndoProtocolData;
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      console.log("[ondo-api] Found tokens:", tokenKeys.length);
      console.log("[ondo-api] Sample tokens:", tokenKeys.slice(0, 15));
      const ousgKey = tokenKeys.find((k) => k.toUpperCase() === "OUSG");
      const usdyKey = tokenKeys.find((k) => k.toUpperCase() === "USDY");
      console.log(
        "[ondo-api] OUSG key:",
        ousgKey,
        "value:",
        ousgKey ? tokens[ousgKey] : "not found"
      );
      console.log(
        "[ondo-api] USDY key:",
        usdyKey,
    ...
    if (
      ...
      const ousgMatches = availableTokens.filter(
        (s) => s.toUpperCase() === "OUSG"
      );
    ...
    console.error("Ondo Finance API error:", error);
    return NextResponse.json({ error: "ondo_fetch_error" }, { status: 500 });
  }
}
