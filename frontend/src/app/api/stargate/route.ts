...
// DefiLlama protocol endpoint for Stargate
// Note: We need to filter for Mantle chain specifically
const STARGATE_PROTOCOL_URL = "https://api.llama.fi/protocol/stargate-v1";

type StargateProtocolData = {
  id?: string;
  name?: string;
  chainTvls?: {
...
export async function GET() {
  try {
    const res = await fetch(STARGATE_PROTOCOL_URL, {
      next: { revalidate: 300 }, // Cache for 5 minutes
    });

    if (!res.ok) {
      return NextResponse.json(
        { error: "stargate_fetch_failed", status: res.status },
        { status: 502 }
      );
    }

    const data = (await res.json()) as StargateProtocolData;
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      console.log("[stargate-api] Found tokens:", tokenKeys.length);
      console.log("[stargate-api] Sample tokens:", tokenKeys.slice(0, 15));
      ...
      console.log(
        "[stargate-api] USDC key:",
        usdcKey,
        "value:",
        usdcKey ? tokens[usdcKey] : "not found"
      );
      console.log(
        "[stargate-api] USDY key:",
        usdyKey,
        "value:",
